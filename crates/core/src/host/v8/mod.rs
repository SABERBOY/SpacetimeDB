...
use v8::{
    ArrayBuffer, Context, Function, Isolate, Local, MapFnTo, OwnedIsolate, PinScope, ResolveModuleCallback, ScriptOrigin, Value, scope_with_context
};
...
impl WasmInstance for V8Instance<'_, '_, '_> {
    ...
    fn call_reducer(&mut self, op: ReducerOp<'_>, budget: FunctionBudget) -> ReducerExecuteResult {
        common_call(self.scope, budget, op, |scope, op| {
            Ok(call_call_reducer(scope, self.hooks, op, self.reducer_args_array_buffer)?)
        })
