...
```
  ...
  <Step title="Understand the provider pattern">
    <StepText>
      ...
      The template uses environment variables for configuration. Set `VITE_SPACETIMEDB_URI` and `VITE_SPACETIMEDB_MODULE` to override defaults.
    </StepText>
    <StepCode>
```tsx
// app/providers.tsx
...
const MODULE = import.meta.env.VITE_SPACETIMEDB_MODULE ?? 'my-remix-app';
